#!/usr/bin/env python3
import argparse, concurrent.futures, ipaddress, json, os, re, socket, subprocess, time
from pathlib import Path

MAC_RE = re.compile(r"([0-9a-fA-F]{2}[:-]){5}[0-9a-fA-F]{2}")
IP_RE = re.compile(r"\b(?:\d{1,3}\.){3}\d{1,3}\b")
OUI = {"b8:27:eb":"Raspberry Pi","dc:a6:32":"Raspberry Pi","e4:5f:01":"Raspberry Pi","52:54:00":"QEMU/KVM","bc:24:11":"Proxmox/QEMU","02:42":"Docker","00:15:5d":"Hyper-V","00:50:56":"VMware","00:0c:29":"VMware","00:11:32":"Synology","00:1d:0f":"QNAP","24:5e:be":"QNAP","00:25:90":"Supermicro","f8:b1:56":"Intel","00:e0:4c":"Realtek","ec:fa:bc":"TP-Link","50:c7:bf":"TP-Link","cc:2d:e0":"Xiaomi","3c:7c:3f":"Apple","f0:18:98":"Apple","a4:83:e7":"Apple"}
...
def main():
    ...
    if len(hosts)>args.max_hosts: raise SystemExit(f"too many hosts: {len(hosts)} > {args.max_hosts}")
    ports=[]
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(128,max(16,len(hosts)))) as ex:
        futs=[ex.submit(scan_one,str(ip),ports,args.timeout_ms,args.ping) for ip in hosts]
    ...
    out={"source":"true-host-script","script_context":"container" if in_container else "ubuntu-host","cidr":str(net),"created_at":time.strftime("%Y-%m-%dT%H:%M:%S%z"),"elapsed_sec":round(time.time()-started,2),"count":len(results),"mac_count":len([r for r in results if r.get("mac_address")]),"arp_count":len(arp),"ports":ports,"results":results}
    out_path=Path(args.out); out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(out,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    print(json.dumps({"out":str(out_path),"source":out["source"],"script_context":out["script_context"],"count":out["count"],"mac_count":out["mac_count"],"arp_count":out["arp_count"],"elapsed_sec":out["elapsed_sec"]},ensure_ascii=False))
