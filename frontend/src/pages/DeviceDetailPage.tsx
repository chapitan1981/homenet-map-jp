...
export default function DeviceDetailPage() {
  ...
  const load=async()=>{
    const [d,p,n,t,ph,cf]=await Promise.all([
  ...
  return <>
    ...
    <div className="card"><h3>カスタム項目</h3><table className="table"><thead><tr><th>項目名</th><th>タイプ</th><th>値</th></tr></thead><tbody>{customFields.map(f=><tr key={f.id}><td>{f.field_name}</td><td>{f.field_type}</td><td>{renderCustomValue(f)}</td></tr>)}{customFields.length===0&&<tr><td colSpan={3}>カスタム項目未登録</td></tr>}</tbody></table></div>
